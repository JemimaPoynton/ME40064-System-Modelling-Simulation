...
for k = 1:length(n)
    N = n(k);
    solver = solverScheme(1/2, 4, N);
    for i = 1:length(x)
        el = x(i);
        mesh = OneDimLinearMeshGen(0, x_length, el);
        mesh.DVec = D*ones(1,mesh.ngn);

        c = solveTransientDiffReact_qb(mesh, lambda, f, BCs, solver);
        c = c(:,1:2:end);
...
%% Plot
figure()
scatter(bound(:,2), bound(:,1), 40, [0.55,0.55,0.55], 'x')
hold on
scatter(non_osc(:,2), non_osc(:,1), 20, [0,0,0], 'o')
dx = 1./(2:0.1:40);
dt = 0.5*dx.^2;
...
plot(dx, dt,'-black')
legend('Ocillatory (Crank Nicholson)', 'Non-Oscillatory (Crank Nicholson)','K = 0.5', 'location', 'northwest')
grid on